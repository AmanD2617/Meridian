...
function AgentGraph({ activeLink }) {
  const nodes = {
    ...
    risk:         { x: 20,  y: 150, agent: "risk",         name: "Risk Agent",         model: "gemini-1.5-flash", status: "streaming" },
    route:        { x: 20,  y: 260, agent: "route",        name: "Routing Agent",     model: "gemini-1.5-pro",   status: "idle" },
      ...
      {Object.values(nodes).map(n => (
        <div key={n.agent} className="agent-node" data-agent={n.agent}
          ...
          <span className="an-tag" style={{
            background: `var(--${n.agent === "weather" ? "warn" : n.agent === "risk" ? "alert" : n.agent === "route" ? "accent" : "violet"}-dim, var(--bg-3))`,
            color: n.agent === "weather" ? "var(--warn)" : n.agent === "risk" ? "var(--alert)" : n.agent === "route" ? "var(--accent)" : "var(--violet)",
          }}>{n.agent}</span>
...
const BASE_MESSAGES = [
  ...
  {
    t: "14:31:53.204", from: "route",
    body: <>INVOKE <span className="key">routing.evaluate_alternates</span>(n=<span className="num">4</span>)
      <pre>{`[
  ...
  },
  {
    ...
    body: <>RECEIVE <span className="arrow">←</span> weather | risk | route <span className="arrow">→</span> <span className="kw">generateSmartReroute</span>()
...
// Awaiting JSON response...`}</pre></>,
  ...
  {
    t: "14:32:01.223", from: "risk",
    body: <>POLL <span className="key">port.rotterdam.queue</span> <span className="arrow">→</span> depth=<span className="num">38h</span> (baseline <span className="num">14h</span>)</>,
  },
  {
...
function AgentActivityView() {
  ...
  React.useEffect(() => {
    const pool = [
      ...
      { from: "risk",    body: <>SCAN <span className="key">port.shenzhen.dwell</span> <span className="arrow">→</span> <span className="num">16.4h</span> (baseline)</> },
      { from: "route",   body: <>HEARTBEAT <span className="key">google_maps.directions</span> · quota=<span className="num">92%</span> remaining</> },
      { from: "orchestrator", body: <>IDLE <span className="arrow">→</span> watching 2,847 shipments · 12ms tick</> },
      { from: "risk",    body: <>ALERT <span className="key">congestion.update</span>(HZ-019) severity=<span className="kw">MEDIUM</span></> },
